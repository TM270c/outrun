...
import { GLRenderer } from './render/glRenderer.js';
import { loadAllTextures, createSpriteMeta, getKindScale } from './render/assets.js';
import { padQuad, tessellateStrip, defaultOverlap } from './render/stripTessellation.js';
import { splitLines, splitCells, toInt, toFloat, toBool, isBoolToken } from './world/csvUtils.js';
...
// ---------- Canvas / GL ----------
  ...
  // ---------- Assets ----------
  const textures = await loadAllTextures(glr);
  const SPRITE_META = createSpriteMeta(glr, textures);
  ...
  async function buildCliffsFromCSV_Lite(url){
    ...
    for (const raw of lines){
      ...
      const aDx   = toFloat(c[3], 0), aDy = toFloat(c[4], 0);
      const bEase = getEase01(c[5]||'smooth:io');
      const bDx   = toFloat(c[6], 0), bDy = toFloat(c[7], 0);
  ...
  // ---------- Overlap (shared) ----------
  const OVERLAP = defaultOverlap; // screen-space pixels to overdraw
  ...
  function drawRoadStrip(x1,y1,w1, x2,y2,w2, v0, v1, fogRoad, tex){
    ...
    const rows = Math.max(1, Math.min(ROAD_SUBDIVISION.rowMax, Math.ceil(dy / ROAD_SUBDIVISION.rowPxTarget)));
    const avgW = 0.5 * (w1 + w2);
    let cols = Math.max(1, Math.round(avgW / ROAD_SUBDIVISION.colPxTarget));
    cols = clamp(cols, ROAD_SUBDIVISION.colsFar, ROAD_SUBDIVISION.colsNear);
    ...
    tessellateStrip({
      ...
      const uvQuad = { u1: uv.u0, v1: uv.v0, u2: uv.u1, v2: uv.v0, u3: uv.u1, v3: uv.v1, u4: uv.u0, v4: uv.v1 };
      const fogQuad = [fog.a, fog.a, fog.b, fog.b];
      glr.drawQuadTextured(tex, quad, uvQuad, DEFAULT_COLORS.road, fogQuad);
    });
  }
  ...
  function drawBoostZonesOnStrip(zones, xNear, yNear, xFar, yFar, wNear, wFar, fogRoad){
    ...
    for (const zone of zones){
      ...
      let cols = Math.max(1, Math.round(0.5 * (nearWidth + farWidth) / ROAD_SUBDIVISION.colPxTarget));
      cols = clamp(cols, ROAD_SUBDIVISION.colsFar, ROAD_SUBDIVISION.colsNear);
      ...
      tessellateStrip({
        ...
        const fogQuad = [fog.a, fog.a, fog.b, fog.b];
        if (tex) {
          glr.drawQuadTextured(tex, quad, uvQuad, solid, fogQuad);
        } else {
          glr.drawQuadSolid(quad, solid, fogQuad);
        }
      });
    }
  }
  ...
  function updatePhysics(dt){
    ...
    let profile = groundProfileAt(phys.s);
    ...
    if (phys.grounded) {
      ...
      profile = groundProfileAt(phys.s);
      phys.y  = profile.y;

      profile = groundProfileAt(phys.s);
      const { dy: ndy, d2y } = profile;
      const kap = computeCurvature(ndy, d2y);
      const slopeInfo = tangentNormalFromSlope(ndy);
      if (kap < 0) {
        const need = phys.vtan * phys.vtan * -kap;
        const support = TUNE_PLAYER.gravity * slopeInfo.ny;
        if (need > support) {
          phys.grounded = false;
          phys.vx = phys.vtan * slopeInfo.tx;
          phys.vy = phys.vtan * slopeInfo.ty;
        }
      }
    } else {
      // airborne
